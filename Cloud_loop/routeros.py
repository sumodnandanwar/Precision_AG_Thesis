#!/usr/bin/python3
# -*- coding: latin-1 -*-
import sys, binascii, socket, ssl
import hashlib
...
class RouterOS:
    def __init__(self, ip, logger=None):
        ...
        s = None
        s = self.open_socket(dst, port, secure)
        if s is None:
            self.logger.info('could not open socket')
            return

        self.apiros = ApiRos(s);
        if not self.apiros.login(user, passw):
            return
    ...
    def open_socket(self, dst, port, secure=False):
        ...
        if secure:
            ...
            s = skt
        self.logger.info("Connecting to RouterOS - Wait up to 2 seconds")
        s.connect(sockaddr)
        return s

    def getWiFiRBridgeMACAddress(self, interfaceName='wlan1'):
        r = self.apiros.talk(['/interface/print','?name=wlan1'])
        return r[0][1]['=mac-address']

    def getWiFiRegisteredClientsMeasurements(self, filterByIP=None, filterByMac=None):
        '''
        ...
        '''
        r = self.apiros.talk(['/interface/wireless/registration-table/print', '=stats'])
        measurements = []
        for response in r[:-1]:
            if filterByIP is not None:
                if response[1]['=last-ip'] == filterByIP:
                    measurements.append(response[1])
            elif filterByMac is not None:
                if response[1]['=mac-address'] == filterByMac:
                    measurements.append(response[1])
            else:
                measurements.append(response[1])
        return measurements